...
class SharedContentScreenTest {
     ...
     */
    private fun verifyDefaultState(
        descriptionOfCheckedBox: String,
        descriptionOfUncheckedBox: String,
    ) {
        ...
        composeTestRule.onAllNodesWithContentDescription(descriptionOfUncheckedBox)
            .assertAll(isNotEnabled())
        composeTestRule.onAllNodesWithContentDescription(descriptionOfCheckedBox)
            .assertAll(isNotEnabled())
    }
    ...
    fun test_long_press_on_one_directory() {
        ...
        composeTestRule.onNodeWithTag(LAZY_COLUMN_TAG)
            .onChildren()[0].performTouchInput { longClick() }
        stateRestorationTester.emulateSavedInstanceStateRestore()
        composeTestRule.onAllNodesWithContentDescription(descriptionOfUncheckedBox)
            .assertCountEquals(MAX_DIRS - 1)
        composeTestRule.onAllNodesWithContentDescription(descriptionOfCheckedBox)
            .assertCountEquals(1)
    ...
    fun test_long_press_on_all_directory() {
        ...
        composeTestRule.onNodeWithTag(LAZY_COLUMN_TAG).onChildren().assertCountEquals(MAX_DIRS)
        composeTestRule.onAllNodesWithTag(CHECK_BOX).assertAll(isEnabled())
            .assertCountEquals(MAX_DIRS)
        composeTestRule.onAllNodesWithContentDescription(descriptionOfCheckedBox)
            .assertCountEquals(MAX_DIRS).assertAll(isEnabled())
        composeTestRule.onAllNodesWithContentDescription(descriptionOfUncheckedBox)
            .assertCountEquals(0)
